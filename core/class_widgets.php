 ...
 class ChildPropertiesWidget extends WP_Widget {
    ...
    function widget($args, $instance) {
		...
		foreach($attachments as $attached):
			...
			$image_sizes = $wp_properties['image_sizes'][$this_property->$image_type];
			?>
			<div class="apartment_entry clearfix">

				<a class="sidebar_property_thumbnail"  href="<?php echo $this_property->permalink; ?>">
					<?php if(!empty($this_property->$image_type)): ?>
					<img width="<?php echo $image_sizes[width]; ?>" height="<?php echo $image_sizes[height]; ?>" src="<?php echo $this_property->$image_type;?>" alt="<?php echo $this_property->post_title; ?> at <?php echo $this_property->location; ?> for <?php echo  $this_property->price; ?>" />
					<?php endif; ?>
				</a>
				...
				<?php foreach($stats as $stat):
					...
					if(empty($content))
						continue;
				?>
					<li><span><?php echo $wp_properties['property_stats'][$stat]; ?>:</span> <?php echo  $content;  ?></li>
				<?php endforeach; ?>
				<?php endif; ?>
				</ul>
		   </div>
          ...
          ?>
            ...
            <p>
				...
				<select id="<?php echo $this->get_field_id('image_type'); ?>" name="<?php echo $this->get_field_name('image_type'); ?>" >
					<?php foreach($wp_properties['image_sizes'] as $name => $sizes) { ?>
						<option value='<?php echo $name; ?>' <?php if($image_type == $name) echo 'SELECTED'; ?>><?php echo $sizes[width]; ?>px by <?php echo $sizes[height]; ?>px</option>
					<?php } ?>
				</select>
			</label>
 ...
 */
 class FeaturedPropertiesWidget extends WP_Widget {
    ...
    function widget($args, $instance) {
		...
		$all_featured = WPP_F::get_featured("return=ID");
 ...
 */
 class SearchPropertiesWidget extends WP_Widget {
    ...
    function widget($args, $instance) {
		...
		draw_property_search_form($searchable_attributes);
    ...
    function form($instance) {
        ...
        $title = esc_attr($instance['title']);
 		$searchable_attributes = $instance['searchable_attributes'];
		$all_searchable_attributes = $wp_properties['searchable_attributes'];
 ...
 */
 class GalleryPropertiesWidget extends WP_Widget {
    ...
    function widget($args, $instance) {
		...
		echo do_shortcode("[gallery size=$image_type columns=1 link=file]");
