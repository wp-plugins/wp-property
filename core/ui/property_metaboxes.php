			...
			<tr class="wpp_attribute_row wpp_attribute_row_<?php echo $slug; ?> <?php if(is_array($wp_properties['hidden_attributes'][$property['property_type']]) && in_array('parent', $wp_properties['hidden_attributes'][$property['property_type']])) echo 'disabled_row;'; ?>">
				...
				<td class="wpp_attribute_cell">
					...
					<?php
						...
						echo apply_filters("wpp_property_stats_input_$slug", "<input type='text' id='wpp_meta_{$slug}' name='wpp_data[meta][{$slug}]'  class='text-input' value='".get_post_meta($object->ID, $slug, true)."' />", $slug, $object);
					?>
					...
					if($common_values): ?>
						<span class="wpp_prefill_values clearfix">
							<span class="wpp_show_prefill_values wpp_link"><?php _e('Show common values.','wpp') ?></span>
							<?php foreach($common_values as $meta): ?>
							<span class="wpp_prefill_attribute"><?php echo($meta); ?></span>
							<?php endforeach; ?>
							<span class="wpp_show_prefill_values_cancel wpp_subtle_link hidden"><?php _e('Cancel.','wpp') ?></span>
						</span>
					<?php endif; ?>
