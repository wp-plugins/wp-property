...
class WPP_Core {
	...
	function WPP_Core(){
		...
		wp_register_script('wp-property-admin-overview', WPP_URL. '/js/wp-property-admin-overview.js', array('jquery'), '0.60' );
		wp_register_script('wp-property-global', WPP_URL. '/js/wp-property-global.js', array('jquery'), '0.60' );
		...
		if ( file_exists( TEMPLATEPATH . '/wp_properties.css') ) {
			wp_register_style('wp-property-frontend', get_bloginfo('template_url') . '/wp_properties.css',   array(),'0.60' );			
		} elseif (file_exists( WPP_Templates . '/wp_properties.css') && $wp_properties[configuration][autoload_css] == 'true') {
			wp_register_style('wp-property-frontend', WPP_URL . '/templates/wp_properties.css',  array(), '0.60' );			
		}
		
		// Find front-end JavaScript and register the script
		if ( file_exists( TEMPLATEPATH . '/wp_properties.js') ) {
			wp_register_script('wp-property-frontend', get_bloginfo('template_url') . '/wp_properties.js', array(),'0.60' );
 		} elseif (file_exists( WPP_Templates . '/wp_properties.js')) {
			wp_register_script('wp-property-frontend', WPP_URL . '/templates/wp_properties.js', array(), '0.60' );
 		}
...
*/
		...
		switch ($column)
		{
			...
			case $wp_properties['configuration']['address_attribute']:
				...
				if($post->address_is_formatted)
					echo "Validated: <a href='http://maps.google.com/maps?q={$property[latitude]},+{$property[longitude]}+%28" . str_replace(" ", "+",$post->post_title). "%29&iwloc=A&hl=en' target='_blank'>view on map</a>.";
				else
	...
	function template_redirect() {
		...
		if($wp->request == $wp_properties['configuration']['base_slug'] || strpos($post->post_content, "[property_overview"))
			$overview_page = true;
	...
	function shortcode_property_overview($atts = "")  {
		...
		extract(shortcode_atts(array('type' => $default_property_type, 'show_children' => 'true'),$atts));
		...
		if(isset($_REQUEST['wpp_search']))
			...
			$properties = WPP_F::get_properties("property_type=$type");
