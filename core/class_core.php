...
class WPP_Core {
	...
	function WPP_Core(){
		...
		wp_register_script('jquery-slider', WPP_URL. '/js/jquery.ui.slider.min.js', array('jquery'), '1.7.3' );
 		wp_register_script('jquery-mouse', WPP_URL. '/js/jquery.ui.mouse.min.js', array('jquery'), '1.4.3' );
	...
	function admin_enqueue_scripts($hook) {
		...
		if($current_screen->id == 'settings_page_property_settings') {
...
*/
	...
	function shortcode_property_overview($atts = "")  {
		...
		$default_property_type = WPP_F::get_most_common_property_type();
                ...
                // mere with specified by user
                $props_atts = array_merge($to_use, $atts);
                                                     
                // get the query string from an array with not empty values
                $c = count($props_atts); $i = 1; $query = '';
                foreach($props_atts as $k => $v){
                    if ($v != '') {
                        if ($k == 'type') $k = 'property_type'; // need this for better UI and to avoid mistakes
                        if ($k == 'per_page') {$per_page = $v; continue; }
                        if ($k == 'starting_row') {$offset = $v; continue; }
                        if ($k == 'sorter') {$sorter = $v; continue; }
                        if ($k == 'ajax_call') {$ajax_call = $v; continue; }
                        $query .= $k.'='.$v;
                        if ($i < $c) $query .= '&';
                    }
                    $i++;
		...
		if(isset($_REQUEST['wpp_search'])){
             	...
             	// Get pagination box and add query limit attributes
    		if(!empty($props_atts['pagination']) || !empty($props_atts['sorter'])) {
    			
		        if(empty($props_atts['ajax_call'])) {
		        	
		        	if(!empty($props_atts['pagination'])) {
        		    //@TODO: need to find another way to get TOTAL count of properties by attributes
        		    $total = count(WPP_F::get_properties($query));
        		     if($total > $limit)
        		        $limit_query = "&limit=$per_page&starting_row=$offset";
	    		    }else{
	    		    	echo 'fdsfs';
	    		    }
                	...
                	if (!empty($wp_properties['property_stats']) && $wp_properties['sortable_attributes']) {
                	    ...
                	    if(!empty($sortable_attrs) && !in_array($order_by, $wp_properties['sortable_attributes'])) {
                	        $order_by = key($sortable_attrs);
                	    }
                	}
