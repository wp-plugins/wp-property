...
class WPP_Core {
	...
	function init() {
		...
		wp_register_script('wp-property-admin-overview', WPP_URL. '/js/wp-property-admin-overview.js', array('jquery'), '1.11' );
		wp_register_script('wp-property-global', WPP_URL. '/js/wp-property-global.js', array('jquery'), '1.11' );
		...
		if ( file_exists( STYLESHEETPATH . '/wp_properties.css') ) {
			wp_register_style('wp-property-frontend', get_bloginfo('stylesheet_directory') . '/wp_properties.css',   array(),'1.11' );
		} elseif (file_exists( TEMPLATEPATH . '/wp_properties.css')) {
			wp_register_style('wp-property-frontend', get_bloginfo('template_url') . '/wp_properties.css',  array(), '1.11' );
		} elseif (file_exists( WPP_Templates . '/wp_properties.css') && $wp_properties['configuration']['autoload_css'] == 'true') {
			wp_register_style('wp-property-frontend', WPP_URL . '/templates/wp_properties.css',  array(), '1.11' );
			...
			if(file_exists( WPP_Templates . "/theme-specific/{$theme_slug}.css")) {
				wp_register_style('wp-property-theme-specific', WPP_URL . "/templates/theme-specific/{$theme_slug}.css",  array('wp-property-frontend'), '1.11' );
			}
		}
		...
		if ( file_exists( STYLESHEETPATH . '/wp_properties.js') ) {
			wp_register_script('wp-property-frontend', get_bloginfo('stylesheet_directory') . '/wp_properties.js', array(),'1.11' );
 		} elseif( file_exists( TEMPLATEPATH . '/wp_properties.js') ) {
			wp_register_script('wp-property-frontend', get_bloginfo('template_url') . '/wp_properties.js', array(),'1.11' );
 		} elseif (file_exists( WPP_Templates . '/wp_properties.js')) {
			wp_register_script('wp-property-frontend', WPP_URL . '/templates/wp_properties.js', array(), '1.11' );
 		}
        ...
        if ($single_page || $wp_query->is_property_overview)	{
			...
			foreach($conditional_styles as $type) {
				...
				if ( file_exists( STYLESHEETPATH . "/wp_properties-{$url_slug}.css") ) {
					wp_register_style('wp-property-frontend-'. $url_slug, get_bloginfo('stylesheet_directory') . "/wp_properties-{$url_slug}.css",   array('wp-property-frontend'),'1.11' );
				} elseif ( file_exists( TEMPLATEPATH . "/wp_properties-{$url_slug}.css") ) {
					wp_register_style('wp-property-frontend-'. $url_slug, get_bloginfo('template_url') . "/wp_properties-{$url_slug}.css",   array('wp-property-frontend'),'1.11' );
				} elseif (file_exists( WPP_Templates . "/wp_properties-{$url_slug}.css") && $wp_properties['configuration']['autoload_css'] == 'true') {
					wp_register_style('wp-property-frontend-'. $url_slug, WPP_URL . "/templates/wp_properties-{$url_slug}.css",  array('wp-property-frontend'), '1.11' );
				}
    ...
    function shortcode_property_overview($atts = "")  {
        ...
        // It's used for multiple shortcode
        if(empty($unique)) {
            static $unique = 100;
        }
        ...
        $query = ''; // get the query string from an array with nt empty values
		
		/* EDIT: The new array used to pass on the variables. Passing a string is not good. */
		$query_array = array();
        ...
        if( empty($props_atts['ajax_call']) ) {
			...
			/* NOT SEARCH i.e we are handling a NORMAL shortcode request */
            if( !isset($_REQUEST['wpp_search']) ) {
			
                $query .= '&pagi='.$offset.'--'.$per_page; // get back limits per page
				
				$query_array['pagi'] = $offset.'--'.$per_page;
				
                $properties = WPP_F::get_properties( $query_array );
                ...
                foreach( $_REQUEST['wpp_search'] as $k => $v ) {
					...
					elseif( isset($v['min']) && ($v['min']=='' || $v['min']=='0' || $v['min']=='-1')  && $v['max']=='' ) {
						...
						if( $v != '' ) {
							...
							// Had to change this to make the search work properly, not sure why urlencode is here.
							//$searchable_attributes[$k] = urlencode($v);
							$searchable_attributes[$k] = $v;
					...
					foreach( $searchable_attributes as $attr => $value ) {
						...
						if( $attr == 'property_type' ) {
                            ...
                            if( is_array( $value ) ) {
								...
								$tmp = (substr($tmp, -1, 1) == ',') ? substr_replace($tmp,'', -1, 1) : $tmp;
								
								$query_array[$attr] = $tmp;
						...
						elseif( is_array( $value ) ) {
							...
							$query_array[$attr] = $value['min'].'-'.$value['max'];
				...
				/* Using Array now, much better. BUT can give some error results, so have to keep an eye on it. */
                $properties = WPP_F::get_properties( $query_array );
            ...
            if(file_exists(STYLESHEETPATH . "/property-pagination-$template.php")) {
                include STYLESHEETPATH . "/property-pagination-$template.php";
            }elseif(file_exists(TEMPLATEPATH . "/property-pagination-$template.php")) {
		...
		/* Triggers when pagination is done via Ajax, i.e Next page is selected */
		else {

			/* Keeping the old variables in there for refrences. But using arrays now. */
        ...
        // Adds template with list of properties
        ob_start();
