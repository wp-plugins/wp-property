...
*/
	...
	function google_maps_infobox($post) {
		...
		<?php if($infobox_settings[show_property_title]  == 'true'): ?>
			<div id="infowindow" style="width:350px; height:200px;" >
		...
		<table cellpadding="0" cellspacing="0" class="wpp_google_maps_infobox_table">
			...
			<tr>
				...
				<td  valign="top">
					...
					<span class="value"><?php echo apply_filters('wpp_stat_filter_'. $attribute_slug, $post->{$attribute_slug}, 'google_map_infobox'); ?></span>
					</li>
					<?php endforeach; ?>
	...
	function get_property_map($id) {
		...
		if ( $keys = get_post_custom( $id )) {
			...
			$wp_image_sizes = array('0'=>"map_thumb", '1'=>"$map_image_type");
			$thumbnail_id = get_post_meta( $id, '_thumbnail_id', true );
