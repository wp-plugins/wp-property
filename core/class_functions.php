...
//echo $meta_key. ': comma:"'.print_r($comma_and, true). '"|hyphen:"'.print_r($hyphen_between, true).'"|specific:"'.$specific.'"<br>';
                        ...
                        default:
                            ...
                            if (WPP_F::is_numeric_range($criteria)) {
                                ...
                                if ($specific == 'all') {
                                    if (isset($matching_ids)) {
                                        $matching_id_filter = implode("' OR post_id ='", $matching_ids);
                                        $matching_ids = $wpdb->get_col("SELECT post_id FROM {$wpdb->prefix}postmeta WHERE (post_id ='$matching_id_filter') AND (meta_key = '$meta_key') AND meta_value != ''");
                                    } else {
		...
		$matching_ids = $wpdb->get_col("SELECT ID FROM {$wpdb->prefix}posts WHERE (ID = '" . implode("' OR ID = '", $matching_ids) . "') AND post_status = 'publish' ORDER BY menu_order ASC");
