		...
		if(is_array($maybe_array = unserialize($args)))
			...
			$query = wp_parse_args( $args, $defaults );
               ...
               if (substr_count($query['pagi'], '--')) {
				...
				//if($query['order_by'])
				//	$sql_order_by = $query['order_by'];
				//if($query['order_dir'])
				//	$sql_order_dir = strtoupper($query['order_dir']);
				
				// ORDER BY `$sql_order_by` $sql_order_dir
unset($query['order_by']);unset($query['order_dir']);
                // Go down the array list narrowing down matching properties
                foreach ($query as $meta_key => $criteria) {
                	if ($meta_key == 'pagination'){ continue; };
                	if ($meta_key == 'pagination'){ continue; };
...
//. ': comma:"'.print_r($comma_and, true). '"|hyphen:"'.print_r($hyphen_between, true).'"|specific:"'.$specific.'"<br>';
                            ...
                            if (WPP_F::is_numeric_range($criteria)) {
                                ...
                                if ($specific == 'all' && !$comma_and && !$hyphen_between) {
                                    ...
                                    if(!substr_count($specific, 'meta_value'))
                                            $specific = "meta_value = '$specific'";

                                    if (isset($matching_ids)) {
                                        $matching_id_filter = implode("' OR post_id ='", $matching_ids);
                                        $matching_ids = $wpdb->get_col("SELECT post_id FROM {$wpdb->prefix}postmeta WHERE meta_key = '$meta_key' AND (post_id ='$matching_id_filter') AND $specific");
                                    } else {
                                        $matching_ids = $wpdb->get_col("SELECT post_id FROM {$wpdb->prefix}postmeta WHERE meta_key = '$meta_key' AND $specific");
                                    }
                                }
                               //UD_F::log($wpdb->last_query. " " . print_r($matching_ids, true));
                            }
		...
		$matching_ids = $wpdb->get_col("SELECT ID FROM {$wpdb->prefix}posts WHERE (ID = '" . implode("' OR ID = '", $matching_ids) . "') AND post_status = 'publish' ORDER BY menu_order ASC {$limit_query}");
