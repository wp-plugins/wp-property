...
*/
	...
	function google_maps_infobox($post) {
		...
		<?php if($infobox_settings[show_property_title]  == 'true'): ?>
			<div id="infowindow" style="width:350px;" >
	...
	function get_property_map($id) {
		global $wp_properties;
		$map_image_type = $wp_properties['feature_settings']['supermap']['supermap_thumb'];
		
			if ( !$map_image_type)
				$map_image_type = 'thumbnail';
		...
		if ( $keys = get_post_custom( $id )) {
			...
			if ($thumbnail_id) {
					...
					$this_url = wp_get_attachment_image_src( $thumbnail_id, $map_image_type , true );
					$return[images][$map_image_type] = $this_url[0];
