...
class WPP_F {
	...
	function feature_check($return = false) {
				...
				foreach($response->code as $code) {
					...
					if(@version_compare($current_file[Version], $version) == '-1') {
						...
						if($current_file[Version])
							...
							UD_F::log("WP-Property Premium Feature: {$code->name} updated to version $version.");
		...
		if($return)
			return "Update ran, " . count($updated_features) . " features downloaded: ". implode(', ' , $updated_features). ".";
	}
...
*/
	...
	function get_properties($args = "") {
		...
		// make sure selected posts are published
		$matching_ids = $wpdb->get_col("SELECT ID FROM {$wpdb->prefix}posts WHERE (ID = '" . implode("' OR ID = '", $matching_ids) . "') AND post_status = 'publish'");
