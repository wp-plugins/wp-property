...
class WPP_F {
	...
	function feature_check($return = false) {
		...
		UD_F::log("Ran: " . print_r($response, true));
...
*/
	...
	function get_properties($args) {
		...
		//echo $id . " - (" .$property_obj[bedrooms]. ")" . WPP_F::do_search_conversion('bedrooms',$property_obj[bedrooms], true);
	...
	function get_property($id, $args = false) {
		...
		*/
		if($load_parent == 'true' && $post[post_parent]) {
			...
			$parent_object = WPP_F::get_property($post[post_parent], "get_children=false&load_thumbnail=false");
		...
		if($get_children == 'true') {
			...
			if(count($children) > 0) {
					...
					foreach($children as $child_id) {
						...
						$child_object = WPP_F::get_property($child_id, "load_parent=false&load_thumbnail=false");
...
*/
	...
	function get_featured($args = "") {
		...
		$by_meta = $wpdb->get_col("SELECT post_id FROM {$wpdb->prefix}postmeta WHERE (meta_key = 'featured' AND meta_value = 'true') OR (meta_key = 'featured' AND meta_value = '1')");
		...
		foreach($by_meta as $post_id) {
			$return[$post_id] = WPP_F::get_property($post_id);
		}
