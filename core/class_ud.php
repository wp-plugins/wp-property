 ...
 * Example:
 * <code>
 * echo UD_UI::checkbox("name=send_newsletter&group=settings&value=true&label=If checked, you will receive our newsletters.", $send_newsletter);
 * </code>
 *
 * Copyright 2010 Andy Potanin <andy.potanin@twincitiestech.com>
 *
...
class UD_UI {
	 ...
	 */
	function input($args = '') {
		$defaults = array('name' => '', 'group' => '','special' => '','value' => '', 'type' => 'text', 'class' => false, 'hidden' => false, 'style' => false, 'readonly' => false, 'label' => false);
		extract( wp_parse_args( $args, $defaults ), EXTR_SKIP );
	...
	function print_admin_scripts($args = '') {
		...
		<script type="text/javascript">
			...
			function ud_json_processing(data) {
				...
				// Show and print message								
				jQuery(message_element).show();
				jQuery(message_element).html("<p>Processing...</p>");
	...
	function use_ud_scripts() {
		add_action('admin_head', array('UD_UI', 'print_admin_scripts'));
	}
	...
	function show_log_page() {
		...
		?>
		<div class="wrap">
		<h2>UD Log Page for get_option('<?php echo UD_PREFIX . 'log'; ?>');
		<a href="<?php echo admin_url("admin.php?page=ud_log&ud_action=clear_log"); ?>" class="button">Clear Log</a>
		</h2>
		...
		<table class="widefat">
			...
			<tr>
				<th><?php echo UD_F::nice_time($event[0]); ?></th>
				<th><?php echo $event[1]; ?></th>
			</tr>
			<?php endforeach; ?>
			</tbody>
		</table>
...
class UD_F {
  ...
  return array_map(array('UD_F' , 'objectToArray'), $object );
	...
	**/
	function check_cache_table($table_name = 'ud_cache') {
		...
		UD_F::log("Cache table does not exist. Attempting to create:");
		...
		UD_F::log("SQL Ran: $sql , verifying existance.");

				
		// Verify it exists
		if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") == $table_name) {
			UD_F::log("Table created and exists.");
			return true;
		}
			
		UD_F::log("Table not created.");
	...
	function log($event) {
		...
		if(!is_array($this_log)) {
			$this_log = array();
			array_push($this_log, array(time(),"Log Started.")); 			
		}
		
		// Insert event into session
		array_push($this_log, array(time(),$event)); 
	...
	function days_since($date1, $return_number = false) {
		...
		if($days == 0) {	return __('Today', WP_INVOICE_TRANS_DOMAIN); }
	...
	function add_log_page() {
		add_action('admin_menu', create_function('', "add_menu_page('Log', 'Log', 10, 'ud_log', array('UD_UI','show_log_page'));"));	
	}
	 ...
	 * @return string 
	 */	
	function create_slug($content, $args = falase) {
		...
		$content = preg_replace('~[^\\pL0-9_]+~u', '-', $content); // substitutes anything but letters, numbers and '_' with separator
	...
	function json($success, $message) {
		...
		// Log in database
		UD_F::log("Automatically logged failed JSON response: $message");
	 ...
	 */	
	 ...
	 function geo_locate_address($address = false) {
