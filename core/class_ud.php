...
class UD_F {
	...
	**/
	function check_cache_table($table_name = 'ud_cache') {
		...
		UD_F::log(__("Cache table does not exist. Attempting to create:",'wpp'));
		...
		UD_F::log(sprintf(__('SQL Ran: %s , verifying existance.','wpp'), $sql ));

				
		// Verify it exists
		if($wpdb->get_var("SHOW TABLES LIKE '$table_name'") == $table_name) {
			UD_F::log(__("Table created and exists.",'wpp'));
			return true;
		}
			
		UD_F::log(__("Table not created.",'wpp'));
	...
	function days_since($date1, $return_number = false) {
		...
		elseif($days > 1 && $days <= 6) { return ($future ? __(" in $days days ",'wpp') : __(" $days days ago",'wpp')); }
	...
	function add_log_page() {
		add_action('admin_menu', create_function('', "add_menu_page(__('Log','wpp'), __('Log','wpp'), 10, 'ud_log', array('UD_UI','show_log_page'));"));
	}
	...
	function json($success, $message) {
		...
		// Log in database
		UD_F::log(__("Automatically logged failed JSON response: ",'wpp').$message);
