<?php
	...
	function wpp_property_stats_add_sold_or_rented($property_stats) {
		...
		$property_stats[for_sale]= "For Sale";
		$property_stats[for_rent]= "For Rent";
	...
	function wpp_property_stats_input_for_rent_make_checkbox($content, $slug, $object) {
		...
		return "<input type='hidden' name='wpp_data[meta][{$slug}]'  value='false'  /><input type='checkbox' id='wpp_meta_{$slug}' name='wpp_data[meta][{$slug}]'  value='true' $checked /> <label for='wpp_meta_{$slug}'>This is a rental property.</label>";
	}
	
		
	function wpp_property_stats_input_for_sale_make_checkbox($content, $slug, $object) {
		$checked = (get_post_meta($object->ID, $slug, true) == 'true' ? ' checked="true" ': false);
		return "<input type='hidden'  name='wpp_data[meta][{$slug}]'  value='false' /><input type='checkbox' id='wpp_meta_{$slug}' name='wpp_data[meta][{$slug}]'  value='true' $checked /> <label for='wpp_meta_{$slug}'>This property is for sale.</label>";
	}
	...
	function add_display_address($property) {
		...
		if(isset($property[parent_id]) 
			...
			if(get_post_meta($property[parent_id], 'address_is_formatted', true)) {
				...
				$display_address = (!empty($street_number) || !empty($route) ? " {$street_number} {$route}<br /> " : "") 
				. "{$city}, {$state}"
				. (!empty($post_code) ? ", {$postal_code}" : "");
		...
		if($property[address_is_formatted]) {
			...
			$display_address = (!empty($property[street_number]) || !empty($property[route]) ? " {$property[street_number]} {$property[route]}<br /> " : "") . "{$property[city]}, {$property[state]} " 
			. (!empty($property[postal_code]) ? ", {$property[postal_code]} " : "");
