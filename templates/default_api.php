<?php
	...
	add_filter("wpp_stat_filter_range_plus", 'add_range_plus');
   ...
   $phone_number = ereg_replace("[^0-9]",'',$phone_number);
	...
	function add_dollar_sign($content) {
		...
		if ( !is_numeric($content) && substr_count($content, '-')){
			...
			return $currency_symbol . $hyphen_between[0] . ' - ' . $currency_symbol . $hyphen_between[1];
	 ...
	 *
	 */	 
	function add_range_plus($content) {
	    return (int)$content . '+';
	}
