	...
	function draw_stats($args = false){
		...
		if(!$stats)
 		...
 		?>
			...
			<dd class="wpp_stat_dd_<?php echo $tag; ?>"><?php echo apply_filters("wpp_stat_filter_$tag", $value); ?>&nbsp;</dd>
 			<?php endforeach; ?>
...
if(!function_exists('draw_property_search_form')):
	function draw_property_search_form($search_attributes = false, $searchable_property_types = false) {
		...
		?>
            ...
            <form action="<?php echo  UD_F::base_url($wp_properties['configuration']['base_slug']); ?>" method="post">
				...
				if(is_array($searchable_property_types)) foreach($searchable_property_types as $this_property) { ?>
					<input type="hidden" name="wpp_search[property_type][]" value="<?php echo $this_property; ?>" />
				<?php } ?>
				...
				?>
					...
					if(WPP_F::is_numeric_range($search_values[$attrib])) { 
						...
						<select id="wpp_search_input_field_<?php echo $attrib; ?>" class="wpp_search_select_field wpp_search_select_field_<?php echo $attrib; ?>" name="wpp_search[<?php echo $attrib; ?>][specific]" > 
							<option value=""></option>
							<?php foreach($search_values[$attrib] as $value) { ?>
							<option value='<?php echo $value; ?>' <?php if($_REQUEST['wpp_search'][$attrib]['specific'] == $value) echo " selected='true' "; ?>>
				...
				<li>
                    <input type="submit" class="wpp_search_button submit" value="Search" />
				</li>
			</ul>
