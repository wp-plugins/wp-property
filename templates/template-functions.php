...
if(!function_exists('draw_stats')):
	...
	function draw_stats($args = false){
        ...
        foreach($stats as $label => $value){
			...
			$value =  trim(apply_filters("wpp_stat_filter_$tag", $value, $post));
            ...
            <ul class="wpp_search_elements">
                <?php if(is_array($search_attributes)) foreach($search_attributes as $attrib) {
                    ...
                    <li class="seach_attribute_<?php echo $attrib; ?> <?php echo ((!empty($wp_properties['searchable_attr_fields'][$attrib]) && $wp_properties['searchable_attr_fields'][$attrib] == 'checkbox') ? 'wpp-checkbox-el' : ''); ?>">
                        ...
                        <label class="wpp_search_label wpp_search_label_<?php echo $attrib; ?>">Type:</label>
                        <?php else : ?>
                        <label class="wpp_search_label wpp_search_label_<?php echo $attrib; ?>"><?php echo (empty($wp_properties['property_stats'][$attrib]) ? ucwords($attrib) : $wp_properties['property_stats'][$attrib]) ?>:</label>
                        <?php endif; ?>
                        <?php if(!empty($wp_properties['searchable_attr_fields'][$attrib])) : ?>
                            ...
                            <?php switch($wp_properties['searchable_attr_fields'][$attrib]) {
                                case 'input' :
                                    ?>
                                    <input name="wpp_search[<?php echo $attrib; ?>]" value="<?php echo $_REQUEST['wpp_search'][$attrib]; ?>" type="text" />
                                   <?php
                                    break;
                                case 'range_input':
                                    ?>
                                ...
                                case 'range_dropdown':
                                    ...
                                    <select class="wpp_search_select_field wpp_search_select_field_<?php echo $attrib; ?>" name="wpp_search[<?php echo $attrib; ?>][min]" >
                                    <option value="0"><?php _e('Any','wpp') ?></option>
                                ...
                                case 'dropdown':
                                    ?>
                                    <select class="wpp_search_select_field wpp_search_select_field_<?php echo $attrib; ?>" name="wpp_search[<?php echo $attrib; ?>]" >
                                    <option value="-1"><?php _e('Any','wpp') ?></option>
                                    <?php foreach($search_values[$attrib] as $value) { ?>
                                        <option value='<?php echo $value; ?>' <?php if($_REQUEST['wpp_search'][$attrib] == $value) echo " selected='selected' "; ?>>
                                            <?php echo apply_filters("wpp_stat_filter_$attrib", $value); ?>
                                        </option>
                                    <?php } ?>
                                    </select>
                                ...
                                case 'checkbox':
                                    ?>
                                    <input type="checkbox" name="wpp_search[<?php echo $attrib; ?>][checked]" <?php if($_REQUEST['wpp_search'][$attrib]['checked'] == 'on') echo " checked='checked' "; ?> /> 
                                    <?php
                                    break;
                            } ?>
                            ...
                            <?php
                                ...
                                <select class="wpp_search_select_field wpp_search_select_field_<?php echo $attrib; ?>" name="wpp_search[<?php echo $attrib; ?>]" >
                                    ...
                                    <?php foreach($search_values[$attrib] as $key => $value) { ?>
                                        <option value='<?php echo $key; ?>' <?php if($_REQUEST['wpp_search'][$attrib] == $key) echo " selected='true' "; ?>>
                                            <?php echo apply_filters("wpp_stat_filter_$attrib", $value); ?>
                                        </option>
                                    <?php } ?>
                                </select>
