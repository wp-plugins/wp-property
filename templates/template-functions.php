...
if(!function_exists('draw_stats')):
	function draw_stats($args = false){
        ...
        foreach($stats as $label => $value){
			...
			<dd class="wpp_stat_dd_<?php echo $tag; ?> <?php echo $alt ?>"><?php echo apply_filters("wpp_stat_filter_$tag", $value, $post); ?>&nbsp;</dd>
		<?php } ?>
...
if(!function_exists('draw_property_search_form')):
	function draw_property_search_form($search_attributes = false, $searchable_property_types = false, $per_page = false) {
        ...
        if (!empty($search_attributes) && !empty($searchable_property_types))
            $search_values = WPP_F::get_search_values($search_attributes, $searchable_property_types);
        ?>
                    ...
                    <li class="seach_attribute_<?php echo $attrib; ?>">
                        ...
                        <?php if(!empty($wp_properties['searchable_attr_fields'][$attrib])) : ?>
                            ...
                            <?php switch($wp_properties['searchable_attr_fields'][$attrib]) {
                            	case 'input' :
                            	    ?>
                            	    <input name="wpp_search[<?php echo $attrib; ?>]" value="<?php echo $_REQUEST['wpp_search'][$attrib]; ?>" />
                            	    <?php
                            	    break;
                                case 'range_input':
                                    ?>
                                ...
                                case 'range_dropdown':
                                    ?>
                                    <select id="wpp_search_input_field_<?php echo $attrib; ?>" class="wpp_search_select_field wpp_search_select_field_<?php echo $attrib; ?>" name="wpp_search[<?php echo $attrib; ?>][min]" >
                                    ...
                                    <?php foreach($search_values[$attrib] as $value) { ?>
                                        <option value='<?php echo (int)$value; ?>' <?php if($_REQUEST['wpp_search'][$attrib]['min'] == $value) echo " selected='true' "; ?>>
                                            <?php echo apply_filters("wpp_stat_filter_range_plus", $value); ?>
                                        </option>
                                    <?php } ?>
                                	</select>
                                    <?php
